...
export class PeriodoDetallePage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.periodo =
      this.periodoService.historial().find((p) => p.id === id) ?? null;
  }
  ...
  get porcentajeUsado(): number {
    ...
    return Math.min(
      (this.periodo.totalGastado / this.periodo.presupuesto) * 100,
      100
    );
  }
  ...
  get periodoRango(): string {
    ...
    const inicio = new Date(
      this.periodo.fechaInicio + 'T00:00:00'
    ...
    const fin = new Date(
      this.periodo.fechaFin + 'T00:00:00'
    ).toLocaleDateString('es-DO', {
      day: '2-digit',
      month: 'short',
      year: 'numeric',
  ...
  get chartOptions() {
    ...
    const labels = Object.keys(porCat).map(
      (id) => this.categorias.find((c) => c.id === id)?.nombre ?? id
    );
    const series = Object.values(porCat);
    const colors = Object.keys(porCat).map(
      (id) => this.categorias.find((c) => c.id === id)?.color ?? '#999'
    );
